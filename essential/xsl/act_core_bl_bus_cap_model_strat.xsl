					...
					<div id="mainContentContainer">
						...
						<div id="sectionModel">
							<div class="sectionContainerFullWidth">
								...
								<div class="sectionBasicTextFullWidth"> 
<p>The following diagram describes the conceptual business capablities in scope for <xsl:value-of select="$orgName"/>.</p>
									<br/>
									<div id="keyContainer">
										<div class="keyLabel">Strategic Importance:</div>
										...
										<div class="keySample"/>
										<div class="keySampleLabel">None</div></div>
 </div>
								<div class="clear"/>
								<br/>
	...
	<xsl:template match="node()" mode="RenderBusinessCapabilityCell">
		<div class="compModelElementContainer">
                ...
                <xsl:variable name="stratLevel" select="current()/own_slot_value[slot_reference='act_strategic_capability']/value"/>
                <xsl:choose>
			<xsl:when test="$stratLevel = '3'">
				<xsl:call-template name="RenderInstanceLink">
				<xsl:with-param name="theSubjectInstance" select="current()"/>
				<xsl:with-param name="theXML" select="$reposXML"/>
				<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
				<xsl:with-param name="divClass" select="'compModelContent small gradLevel5 textColour7'"/>
			</xsl:call-template>
			</xsl:when>
			<xsl:when test="$stratLevel = '2'">
				<xsl:call-template name="RenderInstanceLink">
				<xsl:with-param name="theSubjectInstance" select="current()"/>
				<xsl:with-param name="theXML" select="$reposXML"/>
				<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
				<xsl:with-param name="divClass" select="'compModelContent small gradLevel3 textColour7'"/>
			</xsl:call-template>
			</xsl:when>
			<xsl:when test="$stratLevel = '1'">
				<xsl:call-template name="RenderInstanceLink">
				<xsl:with-param name="theSubjectInstance" select="current()"/>
				<xsl:with-param name="theXML" select="$reposXML"/>
				<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
				<xsl:with-param name="divClass" select="'compModelContent small gradLevel1'"/>
			</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="RenderInstanceLink">
